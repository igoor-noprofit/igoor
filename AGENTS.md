...
8. **Audio routing**: TTS streams to connected browsers when `is_remote_ui()` is true (headless OR external access); local playback happens otherwise. The stream (play_stream/chunks/play_stream_end) is delivered to the PRIMARY page only — the most recently connected `/ws/app` websocket (`websocket_server.send_*_primary`) — so several open pages never play the same voice on top of each other; when that page disconnects, the hub promotes the next most recent one. The input side follows the same rule: on every primary change the hub fires its `on_primary_change` handler (wired in `fastapi_app.create_app`), which tells each page its `audio_role` over `/ws/app`; app.js re-dispatches it as the `igoor-audio-role` DOM event and asrjs gates the mic on it (continuous VAD, wakeword, speaker-id chunks, mic clicks/shortcuts all run on the primary page only). SAPI/ttsdefault can never stream — it warns and plays on the machine.
