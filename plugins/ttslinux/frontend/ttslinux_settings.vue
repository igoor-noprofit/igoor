<template>
    <div class="ttslinux-plugin-settings form-grid">
        ...
        <!-- Speech Rate -->
        <div class="form-label">{{ t('Speech rate (words per minute)') }}</div>
        <div class="form-input">
            <input type="number" v-model.number="formData.rate" min="80" max="600" step="5" />
        </div>
        <div class="form-note"></div>
    ...
    methods: {
        ...
        async handleSave() {
            try {
                ...
                await this.callPluginRestEndpoint('ttslinux', 'set_voice', {
                    method: 'POST',
                    data: {
...
<style scoped>
.ttslinux-plugin-settings.form-grid {
    display: grid;
